...
//	list<int> cNbFarms <- [5499, 1989, 394];		// 5499 Small farms; 1989 Medium farms; 394 Large farms
	...
	reflex rInitContacts {
		...
		ask sFarm {
			...
			loop i from: 0 to: 2 {
				...
				if (aType = 2 and i = 1) {
					save ("CR: " + (vCRDirect[aType, i] * pDays/7) + " - NbContacts: " + aNbDirect[i]) to: "../includes/verify.txt" type: "text" rewrite: false;
				}
	...
	reflex rPause when: cycle = cDuration[0] * 7/pDays {
		...
		loop i from: 0 to: 2 {
			loop j from: 0 to: 2 {
				if vCRDirect[i,j] > 0 {
					// float vActual <- verifyCRDirect[i,j]/cNbFarms[i]/(cDuration[0] *7)/pDays;
					float vActual <- verifyCRDirect[i,j]/cNbFarms[i]/(cDuration[0]);
				...
				if vCRIndirect[i,j] > 0 {
					// float vActual <- verifyCRIndirect[i,j]/cNbFarms[j]/(cDuration[0] *7)/pDays;
					float vActual <- verifyCRIndirect[i,j]/cNbFarms[j]/(cDuration[0]);
