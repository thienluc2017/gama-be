...
* Author: HuyNQ52
* Tags: Enable infectious duration -> Medium infected farms increase so fast and large
*/
...
//				write name + " " + string(cCRDirect[aType, i]) + " " + string(cCRIndirect[i, aType]);
		...
		/* Update aInfectedDays and Create contacts */
		ask sFarm {
			/* Update aInfectedDays */
			if isInfected {
				aInfectedDays <- aInfectedDays + 1;
				if aInfectedDays > cDuration[aType] + cDelay {
					isInfected <- false;
					aInfectedDays <- 0;
				}
			}
			if isInfected and aInfectedDays = 1 {
				vNew <- vNew + 1;
			}
	...
	output {
		display myChart refresh: every(1 #cycles) {
			chart "Số trang trại bị nhiễm bệnh" {
				...
				data "New" value: vNew color: #blue;
			}
		}
	}
}