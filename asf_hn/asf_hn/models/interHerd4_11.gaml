...
//	float kContact <- 0.5*0.5;
	...
	reflex pause when: cycle = 52*cDays {
//		write sFarm[0].name;
//		write sFarm[0].aListDirectSmall;
//		write sFarm[0].aListDirectMedium;
//		write sFarm[0].aListIndirectSmall;
//		write sFarm[0].aListIndirectMedium;
//		write sFarm[0].aListIndirectLarge;
...
//				save ("Indirect Large: " + string(aTotalIndirectLarge/52)) to: "../includes/verify.txt" type: "text" rewrite: false;
	...
	reflex rContacts {
		write "Week: " + string(int(cycle/7)) + " - Day:" + string(cycle);
//		save("Week: " + string(cycle)) to: "../includes/verify.txt" type: "text" rewrite: false;
		...
		ask sFarm {
			...
			aIndirectLarge <- 0;
			if (aDispSize = 3) {
				aDirectSmall <- poisson(0.072*kContact/cDays);	// Số lần 1 Small farm chuyển lợn sang các Small farms khác trong 1 tuần
				aIndirectSmall <- poisson(0.282*kContact/cDays);	// Số lần 1 Small farm nhận indirect contact từ các Small farms khác trong 1 tuần
				aIndirectMedium <- poisson(0.282*kContact/7);	// Số lần 1 Small farm nhận indirect contact từ các Medium farms trong 1 tuần
			} else if (aDispSize = 6) {
				aDirectSmall <- poisson(0.072*kContact/cDays); 	// Số lần 1 Medium farm chuyển lợn sang các Small farms trong 1 tuần
				aIndirectSmall <- poisson(0.282*kContact/cDays);	// Số lần 1 Medium farm nhận indirect contact từ các Small farms trong 1 tuần
				aDirectMedium <- poisson(0.073*kContact/cDays);	// Số lần 1 Medium farm chuyển lợn sang các Medium farms khác trong 1 tuần
				aIndirectMedium <- poisson(0.271*kContact/cDays);	// Số lần 1 Medium farm nhận indirect contact từ các Medium farms khác trong 1 tuần
				aIndirectLarge <- poisson(0.271*kContact/cDays);	// Số lần 1 Medium farm nhận indirect contact từ các Large farms trong 1 tuần
			} else if (aDispSize = 12) {
				aDirectMedium <- poisson(0.073*kContact/cDays);	// Số lần 1 Large farm chuyển lợn sang các Medium farm trong 1 tuần
				aIndirectMedium <- poisson(3.5*kContact/cDays);	// Số lần 1 Large farm nhận indirect contact từ các Medium farms trong 1 tuần
				aIndirectLarge <- poisson(3.5*kContact/cDays);	// Số lần 1 Large farm nhận indirect contact từ các Large farms khác trong 1 tuần
			} else
			{
//				write "[Error] Invalid farm type: " + string(aDispSize);
			}
...
//			save("[Info] New contacts: " + string(aDirectSmall + aDirectMedium + aIndirectSmall + aIndirectMedium + aIndirectLarge 
//				- length(aListDirectSmall) - length(aListDirectMedium) - length(aListIndirectSmall) - length(aListIndirectMedium) - length(aListIndirectLarge)
//			)) to: "../includes/verify.txt" type: "text" rewrite: false;
		}
...
//		write "[Info] Number of Large infected: " + string(vNbFarmsLargeInfected);
		...
		ask sFarm {
			...
			loop times: aDirectSmall - length(aListDirectSmall) {	
			...
			loop times: aDirectMedium - length(aListDirectMedium) {	
				ask sFarm at_distance cRange {
			...
			loop times: aIndirectSmall - length(aListIndirectSmall) {
				ask sFarm at_distance cRange {
			...
			loop times: aIndirectMedium - length(aListIndirectMedium) {
				ask sFarm at_distance cRange {
			...
			loop times: aIndirectLarge - length(aListIndirectLarge) {
				ask sFarm at_distance cRange {
...
//	    }
	...
	reflex rInfected {
		if !isInfected {
			loop vFarm over: (aIndirectSmall among aListIndirectSmall) {
				if vFarm.isInfected {
					if (aDispSize != 12) {
						if flip(0.6) {
							isInfected <- true;
		...
		if !isInfected {
			loop vFarm over: (aIndirectMedium among aListIndirectMedium) {
				if vFarm.isInfected {
					if (aDispSize != 12) {
						if flip(0.6) {
							isInfected <- true;
...
//							write("Hello infected Large from Mdeium");
		...
		if !isInfected {
			loop vFarm over: (aIndirectLarge among aListIndirectLarge) {
				if vFarm.isInfected {
					if (aDispSize != 12) {
						if flip(0.6) {
							isInfected <- true;
...
//				do die;
//			}
			loop vFarm over: (aDirectSmall among aListDirectSmall) {
				if !vFarm.isInfected {
					if (aDispSize != 12) {
						if flip(0.6) {
			...
			loop vFarm over: (aDirectMedium among aListDirectMedium) {
				if !vFarm.isInfected {
					if (aDispSize != 3) {
						if flip(0.6) {
							vFarm.isInfected <- true;
...
//				data "Culled" value: vNbFarmsCulled/cNbFarmsTotal color: #black;
			...
			chart "Số lượng trang trại bị nhiễm bệnh" type: series {
